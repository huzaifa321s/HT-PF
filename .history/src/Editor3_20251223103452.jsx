...
const PdfPageEditor2 = ({ mode }) => {
  ...
  const handleAddSection = () => {
    ...
    if (newSection.type === "mixed") {
      const blocks = parseMixedContent(newSection.content);
      blocks.forEach((block) => {
        if (block.type === "table") {
          const tableResult = parseSmartTable(block.content);
          if (tableResult) {
            dispatch(
              addTable({
      ...
      dispatch(showToast({ message: "Mixed content structured and added!", severity: "success" }));
    } else {
      dispatch(addSection({ ...newSection, id: generateId() }));
      dispatch(
        showToast({ message: "New section added!", severity: "success" })
      );
    }

    setNewSection({ type: "plain", title: "", content: "" });
    setShowSuccess(true);
      ...
      <Box>
        {blocks.map((block, i) => (
          <Box key={i} sx={{ mb: block.type === 'title' ? 1 : 2 }}>
            {block.type === "table" ? (
              <Box sx={{ border: "1px dashed #ccc", p: 1, borderRadius: 1, bgcolor: "#fff" }}>
                <Typography variant="caption" sx={{ color: "#777", fontWeight: 700, mb: 1, display: "block" }}>
                  DETECTED TABLE
                </Typography>
                    ...
                    {block.content.split("\n").slice(0, 3).map((line, lIdx) => (
                      <TableRow key={lIdx}>
                        {line.split(/\t|,|=|:/).map((cell, cIdx) => (
                          <TableCell key={cIdx}>{cell.trim()}</TableCell>
                        ))}
                      </TableRow>
                    ))}
                  </TableBody>
