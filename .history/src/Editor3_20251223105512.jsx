  ...
  const renderLivePreview = (content, type, title = "") => {
    const renderBoldText = (text) => {
      ...
      return segments.map((seg, i) => (
        <span key={i} style={{ fontWeight: seg.bold ? 700 : 400 }}>
          {seg.text}
        </span>
      ));
    };
    ...
    if (type === "title") {
      return (
        <Box sx={{ mb: 1 }}>
          <Typography variant="h6" sx={{ fontWeight: 800, color: colorScheme.primary, letterSpacing: "-0.5px" }}>
    ...
    if (type === "bullets" || type === "numbered") {
      ...
      return (
        <Box sx={{ mb: 2, pl: 2 }}>
          {title && (
            <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1, color: colorScheme.primary }}>
              {renderBoldText(title)}
            </Typography>
          )}
          {lines.map((line, i) => (
            <Box key={i} sx={{ display: "flex", gap: 1, mb: 0.5 }}>
              <Typography variant="body2" sx={{ color: colorScheme.primary, fontWeight: 700 }}>
                {type === "bullets" ? "•" : `${i + 1}.`}
              </Typography>
              <Typography variant="body2" sx={{ color: "text.secondary", lineHeight: 1.6 }}>
                {renderBoldText(line.replace(/^\d+\.\s*|^\u2022\s*|^\*\s*/, ""))}
              </Typography>
            </Box>
          ))}
        </Box>
    ...
    return <Typography variant="body2">{renderBoldText(content)}</Typography>;
  };
