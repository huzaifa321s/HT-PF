"use client";
import { Paper, Typography, Divider, Box, Avatar, Stack, Tooltip, Fade, Chip, TextField, IconButton, useTheme, useMediaQuery } from "@mui/material";
import EmailTwoToneIcon from "@mui/icons-material/EmailTwoTone";
import ContentCopyTwoToneIcon from "@mui/icons-material/ContentCopyTwoTone";
import { useState } from "react";
import { CheckCircle, ContentCopyOutlined, ContentCopySharp, Edit, InfoOutlined, Save } from "@mui/icons-material";
...
${yourName || "Your Name"}`;
  ...
  return (
    ...
    >
      ...
      >
        <InfoOutlined sx={{ color: '#f59e0b', fontSize: 24 }} />
        <Typography
          variant="body2"
          sx={{
            color: '#2d3748',
        ...
        >
          This is a preview of the email that will be <strong>automatically sent</strong> to the client once the proposal is generated.
          Please note that this automation is currently <strong>not functional</strong>.
        </Typography>
      </Box>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          >
            <EmailTwoToneIcon sx={{ fontSize: { xs: 28, sm: 36 }, color: '#f3a833' }} />
          </Box>
          <Box>
            <Typography
              variant="h5"
        ...
        <Tooltip title={copied ? "Copied!" : "Copy Email"} arrow>
          <Box
            ...
            sx={{
              ...
              justifyContent: 'center',
              borderRadius: '50%',
              bgcolor: copied ? 'rgba(76, 175, 80, 0.15)' : 'rgba(243, 168, 51, 0.15)',
              color: copied ? '#4caf50' : '#f3a833',
              cursor: 'pointer',
              transition: 'all 0.3s ease',
              alignSelf: { xs: 'flex-end', sm: 'center' },
              boxShadow: copied ? '0 4px 20px rgba(76, 175, 80, 0.3)' : '0 4px 20px rgba(243, 168, 51, 0.3)',
              '&:hover': {
                bgcolor: copied ? 'rgba(76, 175, 80, 0.25)' : 'rgba(243, 168, 51, 0.25)',
                transform: 'translateY(-2px)',
                boxShadow: copied ? '0 8px 28px rgba(76, 175, 80, 0.4)' : '0 8px 28px rgba(243, 168, 51, 0.4)',
              },
            }}
          >
            {copied ? <CheckCircle sx={{ fontSize: { xs: 22, sm: 28 } }} /> : <ContentCopyTwoToneIcon sx={{ fontSize: { xs: 22, sm: 28 } }} />}
          </Box>
        </Tooltip>
      </Stack>
      ...
      <Fade in={true}>
        <Box
          sx={{
            p: { xs: 2, sm: 3 },
            borderRadius: 3,
            background: 'linear-gradient(135deg, rgba(243, 168, 51, 0.1) 0%, rgba(245, 158, 11, 0.1) 100%)',
            mb: 3,
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            {isEditingSubject ? (
              <TextField
                ...
                InputProps={{
                  startAdornment: (
                    <Typography sx={{ fontWeight: 700, color: '#f3a833', fontSize: { xs: '0.9rem', sm: '1.1rem' }, mr: 1 }}>
                      📧 Subject:
                    </Typography>
                  ),
                }}
            ...
            <Tooltip title={isEditingSubject ? "Save" : "Edit Subject"} arrow>
              ...
              >
                {isEditingSubject ? <Save sx={{ fontSize: { xs: 20, sm: 24 } }} /> : <Edit sx={{ fontSize: { xs: 20, sm: 24 } }} />}
              </IconButton>
            </Tooltip>
          </Box>
        </Box>
      ...
      <Fade in={true}>
        <Box
          sx={{
            ...
            border: '2px solid rgba(243, 168, 51, 0.15)',
            boxShadow: '0 4px 20px rgba(0,0,0,0.6)',
        ...
        >
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
            <Tooltip title={isEditingBody ? "Save" : "Edit Email"} arrow>
              ...
              >
                {isEditingBody ? <Save sx={{ fontSize: { xs: 20, sm: 24 } }} /> : <Edit sx={{ fontSize: { xs: 20, sm: 24 } }} />}
              </IconButton>
            </Tooltip>
          </Box>
