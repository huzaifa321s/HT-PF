    ...
    >
      <Card sx={cardStyle}>
        <CardContent>
          ...
          <Menu
            ...
            PaperProps={{
              sx: {
                borderRadius: 4,
                mt: 1,
                boxShadow: "0 12px 32px rgba(243, 168, 51,0.25)",
                minWidth: 240,
              },
            }}
          >
            ...
            >
              <Title sx={{ mr: 2, color: "#f3a833" }} /> Heading Block
            </MenuItem>
            <MenuItem
              onClick={() => {
            ...
            >
              <TextFields sx={{ mr: 2, color: "#f3a833" }} /> Text Block
            </MenuItem>
            <MenuItem
              onClick={() => {
            ...
            >
              <FeaturedPlayList sx={{ mr: 2, color: "#f3a833" }} /> Standalone
              Package
            </MenuItem>
            <MenuItem
              onClick={() => {
            ...
            >
              <GridView sx={{ mr: 2, color: "#f3a833" }} /> Grid Package (2 per
              row)
            </MenuItem>
          </Menu>
          ...
          {/* Page Title & Headings */}
          <Box sx={{ mb: 4 }}>
            <Typography variant="h6" fontWeight={800} mb={2} sx={{ fontSize: { xs: "1rem", sm: "1.25rem" } }}>
            ...
            {(!data.elements || data.elements.length === 0) &&
              ...
              <Box
                sx={{
                  p: { xs: 3, sm: 5 },
                  textAlign: "center",
                  border: "2px dashed rgba(243, 168, 51,0.3)",
                  borderRadius: 5,
                  bgcolor: "rgba(243, 168, 51,0.02)",
                }}
              >
                <AddIcon
                  sx={{ fontSize: { xs: 50, sm: 80 }, color: "#f3a833", opacity: 0.4, mb: 2 }}
                />
                <Typography
                  variant="h5"
                ...
                <Typography variant="body2" color="text.secondary" mt={1} sx={{ fontSize: { xs: "0.85rem", sm: "1rem" } }}>
              ...
              <>
                {data.elements?.map((el) => (
                  ...
                  >
                    <CardContent sx={{ p: { xs: 2.5, sm: 4 } }}>
                      {el.type === "package" ? (
                        <MemoStandalonePackage el={el} colorScheme={colorScheme} inputStyle={inputStyle} isMobile={isMobile} />
                      ) : (
                ...
                {data.gridPackages?.map((pkg) => (
                  ...
                  >
                    <CardContent sx={{ p: { xs: 2.5, sm: 4 } }}>
                      <MemoGridPackage pkg={pkg} colorScheme={colorScheme} inputStyle={inputStyle} isMobile={isMobile} />
                    </CardContent>
                    <CardActions
                      sx={{
                        bgcolor: "rgba(243, 168, 51,0.03)",
...
const StandalonePackage = ({ el, colorScheme, inputStyle, isMobile }) => {
  const dispatch = useDispatch();
  ...
  return (
    <Stack spacing={3}>
      ...
      <Typography fontWeight={700} color="#f3a833" sx={{ fontSize: { xs: "0.95rem", sm: "1.05rem" } }}>
        Features
      </Typography>
      <TextField
        fullWidth
        size="small"
        placeholder="Quick Paste: Paste multiple lines/bullets here..."
        onPaste={(e) => {
          const features = parsePastedFeatures(e);
          if (features) {
            e.preventDefault();
            dispatch(addMultipleItemsToStandalonePackage({ elementId: el.id, items: features }));
            dispatch(showToast({ message: `${features.length} features added!`, severity: "success" }));
          }
        }}
        sx={{
          mb: 2,
      ...
      {(el.items || []).map((item, i) => (
        <Box key={i} sx={{ display: "flex", gap: 1 }}>
          <TextField
            ...
            sx={{ "& .MuiOutlinedInput-root": { bgcolor: "#141414" } }}
          />
          <IconButton
            color="error"
      ...
      <Button
        ...
        onClick={addFeature}
        fullWidth={isMobile}
        sx={{ alignSelf: isMobile ? "stretch" : "flex-start", borderRadius: 10, py: { xs: 1, sm: 0.5 } }}
      >
        Add Feature
      </Button>
    </Stack>
...
const GridPackage = ({ pkg, colorScheme, inputStyle, isMobile }) => {
  const dispatch = useDispatch();
  const onField = useCallback(
