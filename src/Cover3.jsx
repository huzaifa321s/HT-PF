...
const PdfPageDocument2 = ({
  orderedSections = [],
  tables = [], // ← New: Multiple tables (up to 3)
}) => {

  // Render Dynamic Sections (same as before)
const renderSection = (sec, idx, isLast) => {
      ...
      <View key={key}>
        ...
        {lines.map((line, i) => (
          <View key={i} style={{ flexDirection: "row", alignItems: "flex-start" }} wrap={false}>
            ...
            <Text style={{ marginRight: 8, fontSize: 11, lineHeight: 1.8, marginTop: 2 }}>•</Text>
        ...
        {blocks.map((block, i) => {
          ...
          return (
            <View key={i} style={{ marginBottom: 16 }}>
              {/* Main numbered point */}
              <Text style={styles.numberedMain}>{i + 1}. {mainText}</Text>
              ...
              {subLines.map((sub, j) => {
                ...
                return (
                  <View
                    ...
                    wrap={false}
                  >
                    <Text style={{ marginRight: 7, fontSize: 10, lineHeight: 1.7, marginTop: 2 }}>•</Text>
                    <Text style={{ flex: 1, fontSize: 10.5, lineHeight: 1.7 }}>
                      {cleanSub}
                    </Text>
                  </View>
                );
  ...
  const renderTable = (table) => {
    ...
    return (
      ...
      >
        {table?.headers?.col1.charAt(0).toUpperCase() + table?.headers?.col1.slice(1).toLowerCase()}
      </Text>
        <View style={styles.tableWrapper}>
          ...
          <View key={row.id || i} style={styles.tableRow} wrap={false}>
            ...
            {/* Second Column (Last cell – no right border) */}
      ...
      <Page size="A4" style={styles.page} wrap>
        ...
        {orderedSections.length > 0 ? (
          orderedSections.map((sec, idx) =>
            renderSection(sec, idx, idx === orderedSections.length - 1)
          )
        ) : (
     ...
     {/* Page 2: All Tables (Only if any table has data) */}
      {tables.length > 0 && tables.some(t => t.rows.length > 0) && (
        <>
          ...
          {/* Render All Custom Tables */}
          {tables.map(table => table.rows.length > 0 && renderTable(table))}

          <Text style={styles.noteText}>
            Final pricing, deliverables & timeline will be confirmed after detailed scope discussion and approval.
          </Text>
