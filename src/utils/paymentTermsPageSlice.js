...
const paymentTermsSlice = createSlice({
  ...
  reducers: {
    ...
    restoreTerm: (state, action) => {
      const mode = state.currentMode;
      const { index, value } = action.payload;
      if (index !== undefined && index >= 0) {
        state[mode].terms.splice(index, 0, value);
      } else {
        state[mode].terms.push(value);
      }
    },
...
export const { setMode4, updateTitle, setPageCount, togglePaymentPageInclusion, addTerm, addMultipleTerms, updateTerm, deleteTerm, restoreTerm, resetTerms, setDBTerms } = paymentTermsSlice.actions;
