...
const page2Slice = createSlice({
  ...
  reducers: {
    ...
    addMultipleSections: (state, action) => {
      const mode = state.currentMode;
      state[mode].orderedSections ??= [];
      const sections = Array.isArray(action.payload) ? action.payload : [];
    ...
    updateSection: (state, action) => {
      ...
      const { id, type, title, content, titleAlign, contentAlign } = action.payload;
    ...
    restoreSection: (state, action) => {
      ...
      if (index !== undefined && index >= 0) {
        state[mode].orderedSections.splice(index, 0, section);
      } else {
        state[mode].orderedSections.push(section);
      }
    },
    ...
    addTable: (state, action) => {
      ...
      const title = action.payload?.title || (columnCount === 3 ? "3-Column Table" : "2-Column Table");
      const payloadHeaders = action.payload?.headers;

      if (action.payload?.T_ID) {
        state[mode].tables.push({
          id: action.payload.T_ID,
          title: action.payload.title || "Table",
          type: action.payload.type === 'quotation' ? 'Quotation' : "",
          columnCount,
          headers: payloadHeaders || (columnCount === 3
      ...
      // Manual page count logic removed
    ...
    restoreTableRow: (state, action) => {
      const mode = state.currentMode;
      const { tableId, row, index } = action.payload;
      const table = state[mode].tables.find((t) => t.id === tableId);
      if (table) {
        if (index !== undefined && index >= 0) {
    ...
    restoreTable: (state, action) => {
      const mode = state.currentMode;
      const { table, index } = action.payload;
      if (index !== undefined && index >= 0) {
        state[mode].tables.splice(index, 0, table);
      } else {
        state[mode].tables.push(table);
      }
    },

    deleteTableColumn: (state, action) => {
      const { tableId, colIndex } = action.payload; // colIndex: 1, 2, or 3
      const mode = state.currentMode;
      const table = state[mode].tables.find((t) => t.id === tableId);
      ...
      // Handle data shifting if middle column is deleted
      if (colIndex === 1) {
        table.headers.col1 = table.headers.col2;
        table.headers.col2 = table.headers.col3;
    ...
    updateColumnFormatting: (state, action) => {
      const mode = state.currentMode;
      const index = state[mode].orderedSections.findIndex((s) => s.id === action.payload);
      if (index > 0) {
        [state[mode].orderedSections[index - 1], state[mode].orderedSections[index]] = [
    ...
    setPageCount: (state, action) => {
      const mode = state.currentMode;
      state[mode].currentPages = action.payload;
      state[mode].pages = action.payload;
    },
