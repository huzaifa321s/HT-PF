...
const page3Slice = createSlice({
  ...
  reducers: {
    ...
    addElement: (state, action) => {
      ...
      const { type, content = '', title = '', desc = '', index } = action.payload;
      ...
      if (type === 'title') {
        ...
        newEl.content = content || '/IMG_0751-1.webp';
        newEl.dimensions = { width: '100%', height: '300px' };
      }

      if (index !== undefined && index >= 0) {
        state[mode].elements.splice(index, 0, newEl);
      } else {
        state[mode].elements.push(newEl);
      }
    },

    editElementContent: (state, action) => {
      const mode = state.currentMode;
      const { id, content, width, height } = action.payload;
      const el = state[mode].elements.find(e => e.id === id);
      if (el && el.type !== 'section') {
        if (content !== undefined) el.content = content;
        if (width !== undefined || height !== undefined) {
          if (!el.dimensions) el.dimensions = {};
          if (width !== undefined) el.dimensions.width = width;
          if (height !== undefined) el.dimensions.height = height;
        }
      }
    },
    ...
    deleteElement: (state, action) => {
      const mode = state.currentMode;
      state[mode].elements = state[mode].elements.filter(el => {
        // Prevent deleting image elements
        if (el.id === action.payload && el.type === 'image') {
          return true;
        }
