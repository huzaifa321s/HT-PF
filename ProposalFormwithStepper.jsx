...
const ProposalFormWithStepper = ({
  ...
  const colorScheme = {
    ...
    brandHoverGradient: "linear-gradient(135deg, #5568d3 0%, #6a3f8f 100%)",
  };
          ...
          <Controller
            ...
            render={({ field }) => (
              <Box>
                ...
                />
                {console.log('existingproposal', existingProposalOwner)}
                {console.log('user.id ', user.id)}
                {errors.clientEmail?.message === "client is already exists with this email" &&
                  existingProposalId &&
                  (user.role === "admin" ||
                    user.id === existingProposalOwner) && (
                    <Button
                      size="small"
                      variant="contained"
                      startIcon={<Preview />}
