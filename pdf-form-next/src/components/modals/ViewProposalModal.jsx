    ...
    <Box sx={{
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
        {icon && React.cloneElement(icon, {
          sx: { fontSize: 18, color: "#f3a833" }
        })}
        <Typography variant="body2" color="text.secondary" sx={{ minWidth: { xs: "unset", sm: 100 }, fontWeight: 600 }}>
          {label}:
        </Typography>
      </Box>
      <Typography variant="body2" sx={{ fontWeight: 500, ml: { xs: 3, sm: 0 } }}>
        {value || "N/A"}
      </Typography>
    </Box>
  );
  ...
  const AccordionSection = ({ panel, title, icon, children }) => (
    <Accordion
      ...
      sx={{
        ...
        borderRadius: 2,
        "&:before": { display: "none" },
        boxShadow: "0 2px 8px rgba(243, 168, 51, 0.08)",
      }}
    >
      <AccordionSummary
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          {React.cloneElement(icon, {
            sx: { fontSize: 22, color: expanded === panel ? "#fff" : "#f3a833" },
          })}
          <Typography sx={{ fontWeight: 600 }}>{title}</Typography>
        </Box>
      </AccordionSummary>
      <AccordionDetails sx={{ p: { xs: 2, sm: 3 }, bgcolor: "#fafbff" }}>
        {children}
      </AccordionDetails>
    </Accordion>
  );
  ...
  return (
    <>
      ...
      >
        ...
        {/* Content */}
        <DialogContent sx={{ p: { xs: 2, sm: 3 }, bgcolor: "#fafbff" }}>
          {isLoading ? (
            <Stack spacing={2}>
              {[...Array(4)].map((_, i) => (
            ...
            <Box sx={{ marginTop: 5 }}>
              ...
              >
                <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2, 1fr)" }, gap: 2 }}>
                  <Paper sx={{ p: 2, bgcolor: "#141414", borderRadius: 2, textAlign: "center" }}>
                    <Typography variant="caption" color="text.secondary">
                      Advance Percent
                    </Typography>
                    <Typography variant="h6" fontWeight={700} color="#f3a833">
